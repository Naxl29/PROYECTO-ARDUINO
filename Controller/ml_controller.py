...
class MiController:  #Controlador para la gestión del modelo ML y el seguimiento del consumo energético
    def __init__(self):
    ...
    def manejar_evento_bombilla(self, bombilla_id: str, estado: bool, consumo_w: float):
        """
        ...
        logging.info(f"💡 Evento: Bombilla {bombilla_id} se ha {'encendido' if estado else 'apagado'} "
                     f"con un consumo de {consumo_w} W.")
        
        # Aquí podrías guardar este evento en la base de datos para análisis futuros
        # self.db.guardar_evento(bombilla_id, estado, consumo_w, datetime.now())

    def obtener_consumo_actual_total(self):   #Calcula el consumo total de todas las bombillas encendidas.
        """
        Returns:
    ...
    def predecir_consumo_proximo_mes(self):  # Predice el valor del consumo para el próximo mes.
        try:
